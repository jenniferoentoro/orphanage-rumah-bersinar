<?php
    ...
    $information = htmlspecialchars($_POST['e']);
    ...
    function uploadInformation($name, $number, $category, $info, $file){
        global $pdo;
        $extension = callExtension($file);
        $new_file_name = $name."_".uniqid().".".$extension;
        $new_file_path = $_SERVER["DOCUMENT_ROOT"].'/pages/assets/bukti_kirim/'.$new_file_name;
        
        if ($category == 1) {

            $sql = "INSERT INTO `data_barang`(`id`, `nama_donatur`, `nomor_telepon`, `lokasi_donasi`, `keterangan`, `bukti_kirim`) VALUES (NULL, ?, ?, ?, ?, ?)";
            $send_data= $pdo->prepare($sql);
            $send_data->execute([$name, $number, 'Panti Asuhan Rumah Bersinar', $info, $new_file_name]);

        } else if ($category == 2) {

            $sql = "INSERT INTO `data_barang`(`id`, `nama_donatur`, `nomor_telepon`, `lokasi_donasi`, `keterangan`, `bukti_kirim`) VALUES (NULL, ?, ?, ?, ?, ?)";
            $send_data= $pdo->prepare($sql);
            $send_data->execute([$name, $number, 'Panti Asuhan Saluran Berkat', $info, $new_file_name]);
    ...
    if (!empty($donator) && !empty($phone) && !empty($donation_loc) && !empty($information) && !is_null($payment)) {
        if (checkNumber($phone) == 1) {
            if (checkFileSize($payment) == 1) {
                if (checkExtension($payment) == 1) {
                    uploadInformation($donator, $phone, $donation_loc, $information, $payment);
                    ...
                    $result = "<div class='col-12 modal-writing mb-2'>
                                    ...
                                    <p class='modal-warning-text'>Format File gambar bukti kirim salah! </p>
                                </div>
                                <br>
                                <div class='col-12 close-wrapper'>
                                    <button type='button' class='close-button-modal' data-bs-dismiss='modal' id='refresh'>Close</button>
                ...
                $result = "<div class='col-12 modal-writing mb-2'>
                                <h3 class='modal-text mb-3'>Donasi Gagal!</h3>
                                <p class='modal-warning-text'>File gambar bukti kirim terlalu besar! </p>
                            </div>
                            <br>
                            <div class='col-12 close-wrapper'>
                                <button type='button' class='close-button-modal' data-bs-dismiss='modal' id='refresh'>Close</button>
